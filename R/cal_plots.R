...
cal_plot <- function(df, outcome, prediction, n_bins = 10, show_loess = FALSE, plot_title = '', ...){
  ...
  g1 <- dplyr::mutate(df, bin = dplyr::ntile(!!parse_expr(prediction), n_bins)) %>%
  ...
  if (show_loess) {
    g1 = g1 +
    ggplot2::geom_smooth(ggplot2::aes(x = !!parse_expr(prediction), y = as.numeric(!!parse_expr(outcome))),
  ...
  g2 <- ggplot2::ggplot(df, ggplot2::aes(x = !!parse_expr(prediction))) +
...
cal_plot_multi <- function(df, outcome, prediction, model, n_bins = 10, show_loess = FALSE, plot_title = '', ...){
  ...
  # The calibration plot
  g1 <- df %>%
    dplyr::group_by(!!parse_expr(model)) %>%
    dplyr::mutate(bin = dplyr::ntile(!!parse_expr(prediction), n_bins)) %>%
    # Bin prediction
    dplyr::group_by(!!parse_expr(model), bin) %>%
    dplyr::mutate(n = dplyr::n(), # Get ests and CIs
           bin_pred = mean(!!parse_expr(prediction), na.rm = TRUE),
           bin_prob = mean(as.numeric(as.character(!!parse_expr(outcome))), na.rm = TRUE),
           se = sqrt((bin_prob * (1 - bin_prob)) / n),
    ...
    if (show_loess == FALSE && n_bins > 0) {
      ...
      g1 = g1 +
        ggplot2::stat_smooth(ggplot2::aes(x = !!parse_expr(prediction), y = as.numeric(!!parse_expr(outcome)),
